...
func (c *Controller) getInfrastructureRoleDefinitions() []*config.InfrastructureRole {
	rolesDefs := c.opConfig.InfrastructureRolesDefs

	// check if we can extract something from the old config option
	if c.opConfig.InfrastructureRolesSecretName != emptyNamespacedName {
		// The old config option could contain either a role description (in
		// the form key1: value1, key2: value2), or just a secret name. Let's
		// try check the description first.
		var roleDef config.InfrastructureRole

		propertySep := ","
		valueSep := ":"

		properties := strings.Split(
			c.opConfig.InfrastructureRolesSecretName.Name,
			propertySep)
